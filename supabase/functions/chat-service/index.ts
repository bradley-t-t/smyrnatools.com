    ...
    try {
        ...
        switch (endpoint) {
            case "fetch-messages": {
                ...
                if (!listItemId) return new Response(JSON.stringify({error: "Missing list item"}), {
                    status: 400,
                    headers: corsHeaders
                });
            ...
            case "send-message": {
                ...
                if (!message) return new Response(JSON.stringify({error: "Empty message"}), {
                    status: 400,
                    headers: corsHeaders
                });
            ...
            case "delete-message": {
                ...
                if (!messageId) return new Response(JSON.stringify({error: "Missing messageId"}), {
                    status: 400,
                    headers: corsHeaders
                });
                if (!userId) return new Response(JSON.stringify({error: "Missing userId"}), {
