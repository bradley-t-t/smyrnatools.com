        ...
        switch (endpoint) {
            ...
            case "fetch-operators": {
                ...
                if (activeError || otherError) {
                    ...
                    return new Response(JSON.stringify({error: err?.message || "Failed to fetch operators"}), {status: 400, headers: corsHeaders});
                }
            ...
            case "get-by-employee-id": {
                ...
                if (!employeeId) return new Response(JSON.stringify({error: "Invalid Employee ID"}), {status: 400, headers: corsHeaders});
                const {data, error} = await supabase.from("operators").select("*").eq("employee_id", employeeId).single();
            ...
            case "update": {
                ...
                const {data, error} = await supabase
                    ...
                    .maybeSingle();
                if (error) return new Response(JSON.stringify({error: error.message}), {status: 400, headers: corsHeaders});
                if (!data) return new Response(JSON.stringify({error: "Operator not found"}), {status: 404, headers: corsHeaders});
                return new Response(JSON.stringify({data}), {headers: corsHeaders});
            }
            case "delete": {
                let body: any; try { body = await req.json(); } catch { body = {}; }
                const employeeId = typeof body?.employeeId === "string" ? body.employeeId : null;
                if (!employeeId) return new Response(JSON.stringify({error: "Invalid Employee ID"}), {status: 400, headers: corsHeaders});
                const {data, error} = await supabase.from("operators").delete().eq("employee_id", employeeId).select("*");
                if (error) return new Response(JSON.stringify({error: error.message}), {status: 400, headers: corsHeaders});
                if (!data || data.length === 0) return new Response(JSON.stringify({error: "Operator was not deleted"}), {status: 404, headers: corsHeaders});
