...
Deno.serve(async (req) => {
    ...
    try {
        ...
        switch (endpoint) {
            case "fetch-all": {
                const { data, error } = await supabase.from("heavy_equipment").select("*").order("identifying_number", {ascending: true});
            ...
            case "fetch-by-id": {
                ...
                if (!id) return new Response(JSON.stringify({error: "Equipment ID is required"}), { status: 400, headers: corsHeaders });
                ...
                const { data: hist, error: histErr } = await supabase.from("heavy_equipment_history").select("changed_at").eq("equipment_id", id).order("changed_at", {ascending: false}).limit(1).maybeSingle();
                if (histErr) return new Response(JSON.stringify({error: histErr.message}), { status: 400, headers: corsHeaders });
                return new Response(JSON.stringify({ data: { ...data, latestHistoryDate: (hist as any)?.changed_at ?? null } }), {headers: corsHeaders});
            }
            case "fetch-active": {
                const { data, error } = await supabase.from("heavy_equipment").select("*").eq("status", "Active").order("identifying_number", {ascending: true});
                if (error) return new Response(JSON.stringify({error: error.message}), { status: 400, headers: corsHeaders });
            ...
            case "fetch-history": {
                ...
                if (!equipmentId) return new Response(JSON.stringify({error: "Equipment ID is required"}), { status: 400, headers: corsHeaders });
            ...
            case "update": {
                ...
                if (!id) return new Response(JSON.stringify({error: "Equipment ID is required"}), { status: 400, headers: corsHeaders });
                if (!userId) return new Response(JSON.stringify({error: "User ID is required"}), { status: 400, headers: corsHeaders });
                const { data: current, error: currentErr } = await supabase.from("heavy_equipment").select("*").eq("id", id).maybeSingle();
                if (currentErr) return new Response(JSON.stringify({error: currentErr.message}), { status: 400, headers: corsHeaders });
                if (!current) return new Response(JSON.stringify({error: "Equipment not found"}), { status: 404, headers: corsHeaders });
                const apiData: Record<string, any> = {
                ...
                const { data, error } = await supabase.from("heavy_equipment").update(apiData).eq("id", id).select().maybeSingle();
                if (error) return new Response(JSON.stringify({error: error.message}), { status: 400, headers: corsHeaders });
                const diffs: Array<{ equipment_id: string; field_name: string; old_value: string | null; new_value: string | null; changed_at: string; changed_by: string; }> = [];
                const fields = [
                ...
                for (const f of fields) {
                    ...
                    if (changed) diffs.push({ equipment_id: id, field_name: f.db, old_value: b?.toString?.() ?? null, new_value: a?.toString?.() ?? null, changed_at: nowIso(), changed_by: userId });
                }
                if (diffs.length) {
            ...
            case "delete": {
                ...
                if (hErr) return new Response(JSON.stringify({error: hErr.message}), { status: 400, headers: corsHeaders });
            ...
            case "fetch-comments": {
                ...
                const equipmentId = typeof body?.equipmentId === "string" ? body.equipmentId : null;
                if (!equipmentId) return new Response(JSON.stringify({error: "Equipment ID is required"}), { status: 400, headers: corsHeaders });
                const { data, error } = await supabase.from("heavy_equipment_comments").select("*").eq("equipment_id", equipmentId).order("created_at", {ascending: false});
                if (error) return new Response(JSON.stringify({error: error.message}), { status: 400, headers: corsHeaders });
                return new Response(JSON.stringify({data: data ?? []}), {headers: corsHeaders});
            }
            case "add-comment": {
                ...
                if (!equipmentId) return new Response(JSON.stringify({error: "Equipment ID is required"}), { status: 400, headers: corsHeaders });
                if (!text) return new Response(JSON.stringify({error: "Comment text is required"}), { status: 400, headers: corsHeaders });
                if (!author) return new Response(JSON.stringify({error: "Author is required"}), { status: 400, headers: corsHeaders });
                const comment = {equipment_id: equipmentId, text, author, created_at: nowIso()};
                const {data, error} = await supabase.from("heavy_equipment_comments").insert([comment]).select().maybeSingle();
                if (error) return new Response(JSON.stringify({error: error.message}), { status: 400, headers: corsHeaders });
                return new Response(JSON.stringify({data}), {headers: corsHeaders});
            }
            case "delete-comment": {
                ...
                if (!commentId) return new Response(JSON.stringify({error: "Comment ID is required"}), { status: 400, headers: corsHeaders });
            ...
            case "fetch-issues": {
                ...
                const equipmentId = typeof body?.equipmentId === "string" ? body.equipmentId : null;
                if (!equipmentId) return new Response(JSON.stringify({error: "Equipment ID is required"}), { status: 400, headers: corsHeaders });
                const { data, error } = await supabase.from("heavy_equipment_maintenance").select("*").eq("equipment_id", equipmentId).order("time_created", {ascending: false});
                if (error) return new Response(JSON.stringify({error: error.message}), { status: 400, headers: corsHeaders });
                return new Response(JSON.stringify({data: data ?? []}), {headers: corsHeaders});
            }
            case "add-issue": {
                ...
                if (!equipmentId) return new Response(JSON.stringify({error: "Equipment ID is required"}), { status: 400, headers: corsHeaders });
                if (!issue) return new Response(JSON.stringify({error: "Issue description is required"}), { status: 400, headers: corsHeaders });
                const allowed = ["Low", "Medium", "High"];
                const severity = allowed.includes(severityIn) ? severityIn : "Medium";
                const id = crypto.randomUUID();
                const {data, error} = await supabase.from("heavy_equipment_maintenance").insert({ id, equipment_id: equipmentId, issue, severity, time_created: nowIso() }).select().maybeSingle();
                if (error) return new Response(JSON.stringify({error: error.message}), { status: 400, headers: corsHeaders });
                return new Response(JSON.stringify({data}), {headers: corsHeaders});
            }
            case "complete-issue": {
                ...
                if (!issueId) return new Response(JSON.stringify({error: "Issue ID is required"}), { status: 400, headers: corsHeaders });
            ...
            case "delete-issue": {
                ...
                const issueId = typeof body?.issueId === "string" ? body.issueId : null;
                if (!issueId) return new Response(JSON.stringify({error: "Issue ID is required"}), { status: 400, headers: corsHeaders });
                const { error, count } = await supabase.from("heavy_equipment_maintenance").delete({count: "exact"}).eq("id", issueId);
                if (error) return new Response(JSON.stringify({error: error.message}), { status: 400, headers: corsHeaders });
                if (!count) return new Response(JSON.stringify({error: "Issue not found or already deleted"}), { status: 404, headers: corsHeaders });
                return new Response(JSON.stringify({success: true}), {headers: corsHeaders});
            }
            case "fetch-by-status": {
                ...
                if (!status) return new Response(JSON.stringify({error: "Status is required"}), { status: 400, headers: corsHeaders });
                const { data, error } = await supabase.from("heavy_equipment").select("*").eq("status", status).order("identifying_number", {ascending: true});
                if (error) return new Response(JSON.stringify({error: error.message}), { status: 400, headers: corsHeaders });
                return new Response(JSON.stringify({data: data ?? []}), {headers: corsHeaders});
            }
            case "search-by-identifying-number": {
                ...
                const { data, error } = await supabase.from("heavy_equipment").select("*").ilike("identifying_number", `%${query}%`).order("identifying_number", {ascending: true});
                if (error) return new Response(JSON.stringify({error: error.message}), { status: 400, headers: corsHeaders });
                return new Response(JSON.stringify({data: data ?? []}), {headers: corsHeaders});
            }
            ...
            case "add-history": {
                ...
                const record = { equipment_id: equipmentId, field_name: fieldName, old_value: oldValue, new_value: newValue, changed_at: nowIso(), changed_by: userId };
                const {data, error} = await supabase.from("heavy_equipment_history").insert(record).select().maybeSingle();
                if (error) return new Response(JSON.stringify({error: error.message}), { status: 400, headers: corsHeaders });
                return new Response(JSON.stringify({data}), {headers: corsHeaders});
            }
            default:
