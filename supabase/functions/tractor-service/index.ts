        ...
        switch (endpoint) {
            case "fetch-all": {
                ...
                const {
                    data: hist,
                    error: histErr
                } = await supabase.from("tractors_history").select("tractor_id, changed_at").order("changed_at", {ascending: false});
                if (histErr) return new Response(JSON.stringify({error: histErr.message}), {
                    status: 400,
                    headers: corsHeaders
                });
                const latestMap: Record<string, string> = {};
                for (const h of hist || []) if (!latestMap[h.tractor_id] || new Date(h.changed_at) > new Date(latestMap[h.tractor_id])) latestMap[h.tractor_id] = h.changed_at;
            ...
            case "fetch-by-id": {
                ...
                const {
                    data: hist,
                    error: histErr
                } = await supabase.from("tractors_history").select("changed_at").eq("tractor_id", id).order("changed_at", {ascending: false}).limit(1).maybeSingle();
                if (histErr) return new Response(JSON.stringify({error: histErr.message}), {
                    status: 400,
                    headers: corsHeaders
                });
