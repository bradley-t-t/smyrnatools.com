    ...
    try {
        ...
        switch (endpoint) {
            ...
            case "fetch-by-id": {
                ...
                return new Response(JSON.stringify({ data: { ...data, latestHistoryDate: hist?.changed_at ?? null } }), {headers: corsHeaders});
            }
            case "fetch-active": {
                const { data, error } = await supabase.from("tractors").select("*").eq("status", "Active").order("truck_number", {ascending: true});
                if (error) return new Response(JSON.stringify({error: error.message}), { status: 400, headers: corsHeaders });
            ...
            case "update": {
                ...
                if (!id) return new Response(JSON.stringify({error: "Tractor ID is required"}), { status: 400, headers: corsHeaders });
                if (!userId) return new Response(JSON.stringify({error: "User ID is required"}), { status: 400, headers: corsHeaders });
                const { data: current, error: currentErr } = await supabase.from("tractors").select("*").eq("id", id).maybeSingle();
                if (currentErr) return new Response(JSON.stringify({error: currentErr.message}), { status: 400, headers: corsHeaders });
                if (!current) return new Response(JSON.stringify({error: "Tractor not found"}), { status: 404, headers: corsHeaders });
                ...
                const { data, error } = await supabase.from("tractors").update(apiData).eq("id", id).select().maybeSingle();
                if (error) return new Response(JSON.stringify({error: error.message}), { status: 400, headers: corsHeaders });
                const diffs: Array<{ tractor_id: string; field_name: string; old_value: string | null; new_value: string | null; changed_at: string; changed_by: string; }> = [];
                const fields = [
                ...
                for (const f of fields) {
                    ...
                    if (changed) diffs.push({ tractor_id: id, field_name: f.field, old_value: b?.toString?.() ?? null, new_value: a?.toString?.() ?? null, changed_at: nowIso(), changed_by: userId });
                }
                if (diffs.length) {
            ...
            case "fetch-comments": {
                ...
                const tractorId = typeof body?.tractorId === "string" ? body.tractorId : null;
                if (!tractorId) return new Response(JSON.stringify({error: "Tractor ID is required"}), { status: 400, headers: corsHeaders });
                const { data, error } = await supabase.from("tractors_comments").select("*").eq("tractor_id", tractorId).order("created_at", {ascending: false});
                if (error) return new Response(JSON.stringify({error: error.message}), { status: 400, headers: corsHeaders });
                return new Response(JSON.stringify({data: data ?? []}), {headers: corsHeaders});
            }
            case "add-comment": {
            ...
            case "fetch-issues": {
                ...
                const tractorId = typeof body?.tractorId === "string" ? body.tractorId : null;
                if (!tractorId) return new Response(JSON.stringify({error: "Tractor ID is required"}), { status: 400, headers: corsHeaders });
                const { data, error } = await supabase.from("tractors_maintenance").select("*").eq("tractor_id", tractorId).order("time_created", {ascending: false});
                if (error) return new Response(JSON.stringify({error: error.message}), { status: 400, headers: corsHeaders });
                return new Response(JSON.stringify({data: data ?? []}), {headers: corsHeaders});
            }
            case "add-issue": {
                ...
                const {data, error} = await supabase.from("tractors_maintenance").insert({ id, tractor_id: tractorId, issue, severity, time_created: nowIso() }).select().maybeSingle();
                if (error) return new Response(JSON.stringify({error: error.message}), { status: 400, headers: corsHeaders });
                return new Response(JSON.stringify({data}), {headers: corsHeaders});
            }
            case "complete-issue": {
            ...
            case "delete-issue": {
                ...
                const issueId = typeof body?.issueId === "string" ? body.issueId : null;
                if (!issueId) return new Response(JSON.stringify({error: "Issue ID is required"}), { status: 400, headers: corsHeaders });
                const { error, count } = await supabase.from("tractors_maintenance").delete({count: "exact"}).eq("id", issueId);
                if (error) return new Response(JSON.stringify({error: error.message}), { status: 400, headers: corsHeaders });
            ...
            case "fetch-by-operator": {
                ...
                if (!operatorId) return new Response(JSON.stringify({error: "Operator ID is required"}), { status: 400, headers: corsHeaders });
                const { data, error } = await supabase.from("tractors").select("*").eq("assigned_operator", operatorId).order("truck_number", {ascending: true});
                if (error) return new Response(JSON.stringify({error: error.message}), { status: 400, headers: corsHeaders });
                return new Response(JSON.stringify({data: data ?? []}), {headers: corsHeaders});
            }
            case "fetch-by-status": {
                ...
                const { data, error } = await supabase.from("tractors").select("*").eq("status", status).order("truck_number", {ascending: true});
                if (error) return new Response(JSON.stringify({error: error.message}), { status: 400, headers: corsHeaders });
                return new Response(JSON.stringify({data: data ?? []}), {headers: corsHeaders});
            }
            case "search-by-truck-number": {
                ...
                const { data, error } = await supabase.from("tractors").select("*").ilike("truck_number", `%${query}%`).order("truck_number", {ascending: true});
                if (error) return new Response(JSON.stringify({error: error.message}), { status: 400, headers: corsHeaders });
                return new Response(JSON.stringify({data: data ?? []}), {headers: corsHeaders});
            }
            ...
            case "add-history": {
                ...
                const record = { tractor_id: tractorId, field_name: fieldName, old_value: oldValue, new_value: newValue, changed_at: nowIso(), changed_by: userId };
