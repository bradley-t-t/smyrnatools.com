...
Deno.serve(async (req) => {
    ...
    try {
        ...
        switch (endpoint) {
            case "fetch-all": {
                const { data, error } = await supabase.from("trailers").select("*").order("trailer_number", {ascending: true});
                ...
                if (issuesErr) return new Response(JSON.stringify({error: issuesErr.message}), { status: 400, headers: corsHeaders });
                ...
                const { data: comments, error: commentsErr } = await supabase
                    ...
                    .select("trailer_id");
                if (commentsErr) return new Response(JSON.stringify({error: commentsErr.message}), { status: 400, headers: corsHeaders });
            ...
            case "fetch-by-id": {
                ...
                if (!id) return new Response(JSON.stringify({error: "Trailer ID is required"}), { status: 400, headers: corsHeaders });
                ...
                const { data: histTop, error: histErr } = await supabase.from("trailers_history").select("changed_at").eq("trailer_id", id).order("changed_at", {ascending: false}).limit(1).maybeSingle();
                if (histErr) return new Response(JSON.stringify({error: histErr.message}), { status: 400, headers: corsHeaders });
                return new Response(JSON.stringify({ data: { ...data, latestHistoryDate: (histTop as any)?.changed_at ?? null } }), {headers: corsHeaders});
            }
            case "fetch-active": {
                const { data, error } = await supabase.from("trailers").select("*").eq("status", "Active").order("trailer_number", {ascending: true});
                if (error) return new Response(JSON.stringify({error: error.message}), { status: 400, headers: corsHeaders });
            ...
            case "fetch-history": {
                ...
                if (!trailerId) return new Response(JSON.stringify({error: "Trailer ID is required"}), { status: 400, headers: corsHeaders });
            ...
            case "update": {
                ...
                if (!id) return new Response(JSON.stringify({error: "Trailer ID is required"}), { status: 400, headers: corsHeaders });
                if (!userId) return new Response(JSON.stringify({error: "User ID is required"}), { status: 400, headers: corsHeaders });
                const { data: current, error: currentErr } = await supabase.from("trailers").select("*").eq("id", id).maybeSingle();
                if (currentErr) return new Response(JSON.stringify({error: currentErr.message}), { status: 400, headers: corsHeaders });
                if (!current) return new Response(JSON.stringify({error: "Trailer not found"}), { status: 404, headers: corsHeaders });
                const apiData: Record<string, any> = {
                ...
                const { data, error } = await supabase.from("trailers").update(apiData).eq("id", id).select().maybeSingle();
                if (error) return new Response(JSON.stringify({error: error.message}), { status: 400, headers: corsHeaders });
                const diffs: Array<{ trailer_id: string; field_name: string; old_value: string | null; new_value: string | null; changed_at: string; changed_by: string; }> = [];
                const fields = [
                    {db: "trailer_number"},
                    {db: "assigned_plant"},
                    {db: "trailer_type"},
            ...
            case "fetch-comments": {
                ...
                const trailerId = typeof body?.trailerId === "string" ? body.trailerId : null;
                if (!trailerId) return new Response(JSON.stringify({error: "Trailer ID is required"}), { status: 400, headers: corsHeaders });
                const { data, error } = await supabase.from("trailers_comments").select("*").eq("trailer_id", trailerId).order("created_at", {ascending: false});
                if (error) return new Response(JSON.stringify({error: error.message}), { status: 400, headers: corsHeaders });
                return new Response(JSON.stringify({data: data ?? []}), {headers: corsHeaders});
            }
            case "add-comment": {
            ...
            case "fetch-issues": {
                ...
                const trailerId = typeof body?.trailerId === "string" ? body.trailerId : null;
                if (!trailerId) return new Response(JSON.stringify({error: "Trailer ID is required"}), { status: 400, headers: corsHeaders });
                const { data, error } = await supabase.from("trailers_maintenance").select("*").eq("trailer_id", trailerId).order("time_created", {ascending: false});
                if (error) return new Response(JSON.stringify({error: error.message}), { status: 400, headers: corsHeaders });
                return new Response(JSON.stringify({data: data ?? []}), {headers: corsHeaders});
            }
            case "add-issue": {
                ...
                const {data, error} = await supabase.from("trailers_maintenance").insert({ id, trailer_id: trailerId, issue, severity, time_created: nowIso() }).select().maybeSingle();
                if (error) return new Response(JSON.stringify({error: error.message}), { status: 400, headers: corsHeaders });
                return new Response(JSON.stringify({data}), {headers: corsHeaders});
            }
            case "complete-issue": {
            ...
            case "delete-issue": {
                ...
                const issueId = typeof body?.issueId === "string" ? body.issueId : null;
                if (!issueId) return new Response(JSON.stringify({error: "Issue ID is required"}), { status: 400, headers: corsHeaders });
                const { error, count } = await supabase.from("trailers_maintenance").delete({count: "exact"}).eq("id", issueId);
                if (error) return new Response(JSON.stringify({error: error.message}), { status: 400, headers: corsHeaders });
            ...
            case "fetch-by-status": {
                ...
                const { data, error } = await supabase.from("trailers").select("*").eq("status", status).order("trailer_number", {ascending: true});
                if (error) return new Response(JSON.stringify({error: error.message}), { status: 400, headers: corsHeaders });
                return new Response(JSON.stringify({data: data ?? []}), {headers: corsHeaders});
            }
            case "search-by-trailer-number": {
                ...
                const { data, error } = await supabase.from("trailers").select("*").ilike("trailer_number", `%${query}%`).order("trailer_number", {ascending: true});
                if (error) return new Response(JSON.stringify({error: error.message}), { status: 400, headers: corsHeaders });
                return new Response(JSON.stringify({data: data ?? []}), {headers: corsHeaders});
            }
            ...
            case "add-history": {
                ...
                const record = { trailer_id: trailerId, field_name: fieldName, old_value: oldValue, new_value: newValue, changed_at: nowIso(), changed_by: userId };
