        ...
        switch (endpoint) {
            case "fetch-all": {
                ...
                const {
                    data: hist,
                    error: histErr
                } = await supabase.from("trailers_history").select("trailer_id, changed_at").order("changed_at", {ascending: false});
                ...
                for (const h of hist || []) {
                    const id = (h as any).trailer_id;
                    const at = (h as any).changed_at;
                    if (!latestMap[id] || new Date(at) > new Date(latestMap[id])) latestMap[id] = at;
                }
