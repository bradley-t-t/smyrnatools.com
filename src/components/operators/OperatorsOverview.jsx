...
const OperatorsOverview = ({filteredOperators = null, selectedPlant = '', onStatusClick}) => {
    ...
    const updateStatistics = (operatorsData) => {
        const counts = {
            ...
            'No Hire': operatorsData.filter(op => op.status === 'No Hire').length
        };
    ...
    const calculatePlantDistributionByStatus = (operatorsData) => {
        ...
        operatorsData.forEach(operator => {
            ...
            if (['Active', 'Light Duty', 'Pending Start', 'Training', 'Terminated', 'No Hire'].includes(status)) {
            ...
            <div className="overview-grid">
                <div className="overview-card status-card">
                    ...
                    <div className="status-grid">
                        ...
                        <div
                            className="status-item clickable"
                            onClick={() => onStatusClick && onStatusClick('No Hire')}
                            tabIndex={0}
                            role="button"
                            style={{cursor: 'pointer'}}
                        >
                            <div className="status-count">{statusCounts['No Hire'] || 0}</div>
                            <div className="status-label">No Hire</div>
                        </div>
                    </div>
                </div>
