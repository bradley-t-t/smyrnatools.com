...
function PickupTrucksOverview({pickups = []}) {
    const stats = useMemo(() => {
        ...
        ;(pickups || []).forEach(p => {
            if (typeof p.mileage === 'number' && !isNaN(p.mileage)) { mileageSum += p.mileage; mileageCount++ }
            ...
            <div className="overview-grid">
                <div className="overview-card status-card">
                    ...
                    <div className="status-grid">
                        <div className="status-item"><div className="status-count">{stats.total}</div><div className="status-label">Total Pickups</div></div>
                        <div className="status-item"><div className="status-count">{stats.statusCounts.Active || 0}</div><div className="status-label">Active</div></div>
                        <div className="status-item"><div className="status-count">{stats.statusCounts.Stationary || 0}</div><div className="status-label">Stationary</div></div>
                        <div className="status-item"><div className="status-count">{stats.statusCounts['In Shop'] || 0}</div><div className="status-label">In Shop</div></div>
                        <div className="status-item"><div className="status-count">{stats.statusCounts.Spare || 0}</div><div className="status-label">Spare</div></div>
                        <div className="status-item"><div className="status-count">{stats.statusCounts.Retired || 0}</div><div className="status-label">Retired</div></div>
                        <div className="status-item"><div className="status-count">{stats.statusCounts.Sold || 0}</div><div className="status-label">Sold</div></div>
                    </div>
                </div>
                ...
                <div className="overview-card plant-card">
                    ...
                    <div className="plant-distribution-table">
                        <table className="distribution-table">
                            <thead>
                            <tr><th>Make</th><th>Count</th></tr>
                            </thead>
                            <tbody>
                            {stats.makeRows.map(([mk, count]) => (
                                <tr key={mk}><td className="plant-name">{mk}</td><td>{count}</td></tr>
                            ))}
                            {stats.makeRows.length === 0 && (
                                <tr><td colSpan={2} className="inactive-dash no-data">No data</td></tr>
                            )}
                            </tbody>
                        </table>
                    </div>
